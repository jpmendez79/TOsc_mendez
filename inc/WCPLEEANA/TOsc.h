...
#define SCALEASIMOV
#ifdef SCALEASIMOV
for(int i=0;i<matrix_fitdata_newworld.GetNrows();++i){
for(int j=26*7;j<matrix_fitdata_newworld.GetNcols();++j){
double tmp = matrix_eff_newworld_pred(i,j);
matrix_fitdata_newworld(i,j)=tmp*1.3;
}
}
#endif
}
  void Set_asimov2fitdata() { matrix_fitdata_newworld   = matrix_eff_newworld_pred; }
  void Set_mix2fitdata()    { matrix_fitdata_newworld   = matrix_eff_newworld_meas; for(int i=0*26;i<7*26;++i){matrix_fitdata_newworld(0,i)=matrix_eff_newworld_pred(0,i);}}
